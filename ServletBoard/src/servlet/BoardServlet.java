...
public class BoardServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (url.lastIndexOf("insert.do") >= 0) {
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_input.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("insertR.do") >= 0) {
			insert(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_input_result.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("list.do") >= 0) {
			list(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_list.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("view.do") >= 0) {
			view(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_view.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("delete.do") >= 0) {
			delete(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_delete.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("selectOne.do") >= 0) {
			selectOne(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_modify.jsp");
			dispatcher.forward(req, resp);
		...
		} else if (url.lastIndexOf("modifyR.do") >= 0) {
			modify(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_modify_result.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("repl.do") >= 0) {
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_repl.jsp");
			dispatcher.forward(req, resp);

		} else if (url.lastIndexOf("replR.do") >= 0) {
			repl(req);
			dispatcher = req.getRequestDispatcher("index.jsp?inc=./board_servlet/board_repl_result.jsp");
			dispatcher.forward(req, resp);
		}
	}
